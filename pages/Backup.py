...
from Portfolio import BackupPortfolio

st.title("Portfolio Analysis")
temp = pd.read_csv('Data/backup_data.csv', index_col=0, parse_dates=True)
backup_warning = 'You are using backup data from ' + str(temp.index.min()).split(' ')[0] + ' to ' + str(temp.index.max()).split(' ')[0] + '.'
st.success(backup_warning)
...
port = BackupPortfolio(tickers=df['Ticker'],
                        weights=df['Weight'],
...
# -------
rf_rate = pd.read_csv('Data/backup_rfrate.csv', index_col=0, parse_dates=True).iloc[-1].values[0]/100
